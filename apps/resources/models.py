...
class ResourceCategory(BaseModel):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            counter = 1
            while ResourceCategory.objects.filter(slug=unique_slug).exclude(pk=self.pk).exists():
    ...
    def resources_count(self):
        return PaidResource.objects.filter(category=self.name, is_deleted=False).count()
...
class PaidResource(BaseModel):
    ...
    course_name = models.CharField(max_length=255, blank=True, default='', help_text="Associated course / programme name")
    category = models.CharField(max_length=100, default='Question Papers')
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            unique_slug = base_slug
            counter = 1
            while PaidResource.objects.filter(slug=unique_slug).exclude(pk=self.pk).exists():
                unique_slug = f"{base_slug}-{counter}"
                counter += 1
            self.slug = unique_slug
        super().save(*args, **kwargs)
